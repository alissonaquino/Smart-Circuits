...
<div class='container card mt-5' style=" background-color: white " id="principal">
  ...
  <li class="nav-item">
    <a class="nav-link active" href="#">Meus Quizzes</a>
  </li>
</ul>
    </Nav>
...
if(isset($_POST['tamanho'])){
  ...
  $tamanho = '';
   ...
   $query =  "SELECT * FROM quizz WHERE id_usu = $idusu && desabilitar_quizz = '0' && assunto_quizz LIKE '%$assunto%' && dificuldade_quizz LIKE '%$dificuldade%' && tamanho_quizz LIKE '%$tamanho%'";
   $dados = mysqli_query($conn, $query);
   while($linha = mysqli_fetch_assoc($dados)){
       $id = $linha["id_quizz"];
       $tamanho =  $linha["tamanho_quizz"];
       $dificuldade = $linha["dificuldade_quizz"];
       $assunto = $linha["assunto_quizz"];
   ...
   echo " 
      <tr key={id}>
        ...
        <a href='atualizaQuizz.php?id=$id&i=1'>
        <button class='btn btn-sm btn-primary'>Atualizar</button>
        </a>

        <a href='deletarQuizz.php?id=$id&i=1'>
